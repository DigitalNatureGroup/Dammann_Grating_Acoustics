...
trap_translate = [-5*lambda, -2.5*lambda, 0];
ii =1;
...
for jj = 1:length(trap_translate)
    ...
    set(gca,'FontSize',24)
    axis equal
    exportgraphics(gca, ['selected_traps\selected_trap_n_' num2str(unique_trap_n(ii)) 'pat_translate_' num2str(jj) '.png'])
    
    disp(['Mean pressure at the peaks' num2str(mean(sorted_peaks(1:unique_trap_n(ii))),4) ' Pa'])
end
% return
rotate_translate = [22.5, 45, 0];
focal_point = [0,0,0.1];
pre_calc = pre_pressure_calc_2d_pat(XX, YY, focal_point,Trans_x, Trans_y, Trans_z, f0, c0, p_0, k, trans_q, a);
...
%     return
    ...
    set(gca,'FontSize',24)
    axis equal
    exportgraphics(gca, ['selected_traps\selected_trap_n_' num2str(unique_trap_n(ii)) 'pat_rotate_' num2str(jj) '.png'])
